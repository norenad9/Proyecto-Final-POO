...
func _find_specific_prisoners():
	...
	prisoner1 = root.get_node_or_null("Prisoner1")
	if not prisoner1:
		prisoner1 = root.get_node_or_null("prisoner1")
	if not prisoner1:
		prisoner1 = root.get_node_or_null("Prisionero1")
	if not prisoner1:
		prisoner1 = root.get_node_or_null("prisionero1")
	
	# Buscar Prisoner2 (o prisoner2, Prisionero2, prisionero2)
	prisoner2 = root.get_node_or_null("Prisoner2")
	if not prisoner2:
		prisoner2 = root.get_node_or_null("prisoner2")
	if not prisoner2:
		prisoner2 = root.get_node_or_null("Prisionero2")
	if not prisoner2:
		prisoner2 = root.get_node_or_null("prisionero2")
