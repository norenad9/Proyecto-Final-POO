...
var time_left: float = 60.0  # Más tiempo por ser más números
...
func start_game():
	...
	# Generar lista objetivo (7-8 números ordenados)
	var count = randi_range(7, 8)
	var numbers = []
	for i in range(25):  # Pool más grande de números del 1 al 25
		numbers.append(i + 1)
	numbers.shuffle()
	...
	var initial_count = randi_range(2, 3)
	...
	# Agregar distractores hasta tener exactamente 7 números disponibles
	while available_numbers.size() < 7:
		var extra = randi_range(26, 40)
...
func update_displays():
	...
	# Mostrar lista objetivo - con nodos más pequeños si son muchos
	var node_size = 70 if target_list.size() > 7 else 80
	for value in target_list:
		var node = create_node_visual(value, game_colors.success, false, node_size)
	...
	for i in range(current_values.size()):
		var node = create_node_visual(current_values[i], game_colors.node, true, node_size)
...
func create_node_visual(value: int, color: Color, can_delete: bool, size: int = 80) -> Control:
	...
	label.add_theme_font_size_override("font_size", 32 if size == 70 else 36)
...
func close_minigame():
	...
	# Reactivar movimiento del jugador real
